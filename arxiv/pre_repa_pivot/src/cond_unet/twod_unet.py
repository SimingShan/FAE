...
class Unet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, time, z=None, emb_add=None):
        assert x.dim() == 5
        orig_shape = x.shape
        ...
        if z is not None:
            if self.param_conditioning == "scalar":
                ...
                raise NotImplementedError(f"Param conditioning {self.param_conditioning} not implemented")
        if emb_add is not None:        # representation conditioning: rep-MLP output added to cond emb (SSLForPDEs)
            emb = emb + emb_add
