...
import numpy as np
from src.utils.config import load_config, assert_budget, ckpt_path
...
def fae_cmd(cfg, seed, tag, ckpt):
    c = [PY, "scripts/train/train_fae.py", "--mode", cfg.mode, "--dataset", cfg.dataset,
...
def baseline_cmd(cfg, seed, tag, ckpt):
    meth = cfg.method if cfg.method in ("mae", "dino") else "ijepa"
    c = [PY, "scripts/train/train_baseline.py", "--method", meth, "--dataset", cfg.dataset,
    ...
    if cfg.get("res_h"):                                  # native-aspect ViTs (shear 128x256) — fair vs FAE
        c += ["--res_h", str(cfg.res_h), "--res_w", str(cfg.res_w)]
    return c
...
def main():
    ...
    for seed in cfg.seeds:
        ...
        subprocess.run(cmd, check=True, cwd=os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        if args.no_eval:
