...
params = np.stack([alphas, betas], 1)                      # (N, 2) — the inverse-probe target

init_perturbs, outputs = [], []
print(f"=== SW gen: {N_SAMPLES} samples, {NPHI}x{NTHETA} sphere, {STOP_HOURS}h sims -> {OUT} ===", flush=True)
for i in range(N_SAMPLES):
    ...
    np.savez(OUT, params=params[:i + 1], inputs=np.array(init_perturbs), outputs=np.array(outputs))
    print(f"    saved {i+1} samples (vorticity {outputs[-1].shape})", flush=True)
